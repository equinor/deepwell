...
class run_dw:
    ...
    def get_model(self):
            ...
            if len(sys.argv)>1 and len(sys.argv)<=2:
                ...
                return model
            #### Run with two arguments to load and retrain $python main.py -2 -2 for example    
            elif len(sys.argv)>2:
                #####This is for retraining the model, for tensorboard integration load
                #####the tensorboard log from your trained model
                ...
                #Continue training
                model.learn(total_timesteps=10000, callback =eval_callback, reset_num_timesteps=False, tb_log_name='PPO2_400_5th')
                #Save the retrained model
                model.save("ppo2_200k+500k")
                return model
            else:
